...
| **<<Description>>** 
| Start after given date (and optional time) (see {uri-gource-docs}[gource docs] for formats)
|===
...
| **<<Description>>** 
| Stop after given date (and optional time) (see {uri-gource-docs}[gource docs] for formats)
|===
...
WARNING: These variables are highly subject to change, and variables may come and go. This particular API is considered *strictly* experimental and cannot ever be considered stable, the only exception being <<_use_gource_nightly, USE_GOURCE_NIGHTLY>>.
...
==== Gource Variables for Templates: Border
